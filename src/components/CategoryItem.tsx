...
const CategoryItem: FC<NavItemProps> = ({ href, text, alt }) => {
  return (
    <Link href={href} className='relative group hover:hue-rotate-[220deg]'>
      ...
      <span className='absolute top-2/4 left-2/4 -translate-y-1/2 -translate-x-1/2 w-full flex justify-center pl-9 font-cubic text-2.5xl group-hover:text-primary-dk text-white'>
        {text}
      </span>
    </Link>
  );
